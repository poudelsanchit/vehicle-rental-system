...
// Form schema with validation for User (Renter)
const formSchema = z.object({
    ...
    // Driver's License (Required for renters)
    licenseNumber: z.string().min(5, "License number is required"),
    licenseExpiryDate: z.string().min(1, "License expiry date is required"),
    licenseFrontPhoto: z.instanceof(File, { message: "License front photo is required" }),
    licenseBackPhoto: z.instanceof(File),
...
const steps = [
    ...
    { id: 4, name: "Driver's License", fields: ["licenseNumber", "licenseExpiryDate", "licenseFrontPhoto", "licenseBackPhoto"] },
    { id: 5, name: "Emergency Contact", fields: ["emergencyContactName", "emergencyContactPhone", "emergencyContactRelation"] },
]
...
export default function UserVerification() {
    ...
    async function onSubmit(values: FormValues) {
        ...
        try {
            ...
            formData.append('licenseFrontPhoto', values.licenseFrontPhoto)

            // Optional license back photo
            if (values.licenseBackPhoto) {
                formData.append('licenseBackPhoto', values.licenseBackPhoto)
            }
            ...
            // Send request to API
            const response = await fetch('/api/v1/verification/kyc/user', {
                method: 'POST',
                body: formData,
            })
        ...
        <div className="min-h-screen py-12 px-4">
            <div className="max-w-3xl mx-auto">
                ...
                <div className="text-center mb-8">
                    <h1 className="text-4xl font-bold mb-2">User KYC Verification</h1>
                    <p className="text-gray-500">Complete your identity verification to start renting vehicles</p>
                </div>
                ...
                <div className="rounded-lg border shadow-xl p-8">
                    <Form {...form}>
                        <div className="space-y-6">
                            ...
                            {currentStep === 4 && (
                                <div className="space-y-4 animate-in fade-in duration-500">
                                    <h2 className="text-2xl font-semibold text-gray-400 mb-1">Driver's License</h2>
                                    <p className="text-gray-400 mb-4">Valid driver's license is required to rent vehicles</p>
                                    ...
                                    <FormField
                                        ...
                                        render={({ field }) => (
                                            <FormItem>
                                                ...
                                                <FormControl>
                                                    <Input placeholder="Enter your license number" {...field} className="h-12" />
                                                </FormControl>
                                                <FormMessage />
                                            </FormItem>
                                        )}
                                    ...
                                    <ImageUpload
                                        ...
                                        description="Upload clear photo of the front side of your license"
                                        required
                                        value={form.watch('licenseFrontPhoto')}
                                        onChange={(file) => form.setValue('licenseFrontPhoto', file as File, { shouldValidate: true })}
                                        error={form.formState.errors.licenseFrontPhoto?.message}
                                    />

                                    <ImageUpload
                                        label="Driver's License Photo (Back)"
                                        description="Upload clear photo of the back side (optional)"
                                        value={form.watch('licenseBackPhoto')}
                                        onChange={(file) => form.setValue('licenseBackPhoto', file as File, { shouldValidate: true })}
                                        error={form.formState.errors.licenseBackPhoto?.message}
                                    />
                                </div>
                            )}
